...
const getBackendUrl = () => {
  if (typeof window !== "undefined" && window.location.hostname !== "localhost" && window.location.hostname !== "127.0.0.1") {
    // In production, ALWAYS use Nginx reverse proxy on same-origin /api
    // Eliminates Mixed Content (HTTP inside HTTPS) and Port 5000 blocking
    return `${window.location.origin}/api`;
  }

  let envUrl = (import.meta.env.VITE_BACKEND_URL || import.meta.env.VITE_API_URL || "http://localhost:5000/api").toString().trim();
